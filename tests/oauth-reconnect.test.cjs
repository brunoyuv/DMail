...
function fixture(googleBrowserSignInEnabled = true) {
  ...
  const state = { paths: ['read'], replacements: [], creates: [], resumed: [], signIns: [], scheduler: 0,
  ...
  const Host = new Function('RegisteredMailOAuth', 'LoopbackBrowserSignIn', 'TokenCredentials', 'JmapError',
    ...
    { settingsChanged: async () => { state.scheduler++; } }, oauthFailureLabel, oauthFailureCode);
  const ui = Object.assign(new Host(), { active: true, busy: false, generation: 10, oauthPending: false, error: '', account,
    ...
    refreshReader: async () => { state.resumed.push('reader'); }, refreshMailbox: async () => { state.resumed.push('mailbox'); },
...
test('Reconnect verifies the saved login, replaces its credentials in place and resumes the current reader', async () => {
  ...
  assert.deepEqual(state.resumed, ['reader']); assert.equal(state.scheduler, 1);
