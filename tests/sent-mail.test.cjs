...
test('Confirmed Sent keeps attachment identities and names without persisting file references or bytes in mail', () => {
  const attachment = { id: '4cfb7e17-b890-4ee6-8893-a4bd261da346', name: '测试.pdf', contentType: 'application/pdf',
    size: 123, file: 'opaque-private-reference' };
  const mail = sentMessage('sender@example.test', { ...draft, attachments: [attachment] }, submitted, box.id);
  assert.equal(mail.hasAttachment, true);
  assert.deepEqual(mail.attachments, [{ id: attachment.id, name: attachment.name, contentType: attachment.contentType, size: 123, sizeIsEncoded: false }]);
  assert.ok(!JSON.stringify(mail).includes(attachment.file));
});
