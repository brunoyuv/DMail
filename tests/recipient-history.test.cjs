...
test('AccountStore backfill and confirmed-send learning use only the captured account cache', async () => {
  ...
  host.mail = { cachedMessages: async id => { calls.push(['cachedMessages', id]); return [{ mail: mail('sent', ['sent'], [{ name: 'Ada', email: 'ada@example.test' }]) }]; },
  ...
  assert.deepEqual(calls, [['cachedMessages', 'account-a'], ['boxes', 'account-a'], ['list', 'account-a'], ['cachedMessages', 'account-b']]);
