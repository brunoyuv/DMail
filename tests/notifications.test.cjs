...
test('Stopping a check during its final account read prevents a new server request', async () => {
  const { db, store } = await fixture(); const env = environment(store);
  ...
  try {
    ...
    store.get = async id => {
      ...
      if (++reads === 2) stopped = true; // Pending notice read, then pre-check ownership read.
      return value;
    };
    env.cancelled = () => stopped;
    assert.equal(await runNotificationChecks(store, env, 'stopped-check', false, 'a'), false);
    assert.equal(reads, 2); assert.deepEqual(env.events, []);
    assert.equal(await store.acquire('next-owner', env.now()), true, 'Cancellation releases the durable lease');
    await store.release('next-owner');
  } finally { db.close(); }
});

test('Stopping before publication preserves the durable notice for a later check', async () => {
  for (const stopAt of ['permission', 'metadata']) {
    const { db, store } = await fixture(); const env = environment(store); let stopped = false;
    try {
      await store.configure('a', true); const value = await store.get('a');
      await store.complete('a', value.revision, check('baseline'), 1000);
      await store.complete('a', value.revision, check('arrival', 1), 2000);
      env.cancelled = () => stopped;
      env.permitted = async () => { if (stopAt === 'permission') stopped = true; return true; };
      const get = store.get.bind(store);
      store.get = async id => { const current = await get(id); if (stopAt === 'metadata') stopped = true; return current; };
      await deliverPending(store, env, 'a');
      assert.deepEqual(env.events, [], stopAt);
      assert.equal((await get('a')).acknowledgedVersion, 0);
      stopped = false; store.get = get; env.permitted = async () => true;
      await deliverPending(store, env, 'a');
      assert.deepEqual(env.events, ['publish:a']);
      assert.equal((await get('a')).acknowledgedVersion, 1);
    } finally { db.close(); }
  }
});


test('A canceled automatic wait keeps the arrival pending without recording a network failure', async () => {
  const { db, store } = await fixture(); const env = environment(store);
  const { AutomaticMailWorkCancelled } = require('../.tools/test-output/mail/AutomaticMailWork');
  try {
    await store.configure('a', true); const before = await store.get('a');
    env.check = async () => { throw new AutomaticMailWorkCancelled(); };
    assert.equal(await runNotificationChecks(store, env, 'canceled-wait', false, 'a'), false);
    const after = await store.get('a');
    assert.equal(after.failures, before.failures); assert.equal(after.lastAttempt, before.lastAttempt);
    assert.equal(after.cursor, before.cursor); assert.deepEqual(env.events, []);
    assert.equal(await store.acquire('next-owner', env.now()), true, 'Cancellation releases the existing durable lease');
    await store.release('next-owner');
  } finally { db.close(); }
});
