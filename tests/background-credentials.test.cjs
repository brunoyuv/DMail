...
function fixture(options = {}) {
  ...
  const counts = { cache: 0, pictures: 0, closed: 0, keysCleared: 0, assetQueries: 0 };
  const db = { version: 5,
...
test('Notification-only opening performs no schema/cache cleanup and retains a foreground account still being added', async () => {
  ...
  try {
    ...
    assert.equal(f.counts.cache, 0); assert.equal(f.counts.pictures, 0); assert.throws(() => store.mail);
