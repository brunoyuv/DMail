...
function fixture() {
  const state = { now: 1800000000000, permitted: true, leaseBusy: false, owner: '', scheduleError: false,
    ...
    badgeBeginError: false, badgeRecordError: false, opens: 0, closes: 0, permissionGates: [], locked: false };
  const observed = { schedules: [], snapshots: [], stopped: [], credentials: [], published: [], badges: [], badgeRecords: [] };
  ...
  class NativeImapClient {
    ...
    async watchInbox(id) {
      if (!this.started.has(id)) { await this.credentials.authorization(); this.started.add(id); }
    ...
    async checkInbox() {
      await this.credentials.authorization(); state.checks++;
  ...
  return { service, updates, storageValues, state, settings, accounts, observed, badgeCounts, timers, advance, stop, context: {} };
}
...
test('An unsupported watch falls back to the selected interval without retrying each heartbeat', async () => {
  ...
  try {
    ...
    await f.advance(60000); assert.equal(f.state.checks, 2); assert.equal(f.observed.snapshots.length, snapshots);
  } finally { await f.stop(); }
});
...
test('A locked background check uses explicit background access and never starts a foreground watch', async () => {
  ...
  try {
    f.state.locked = true;
    assert.equal(await f.service.run(f.context, true), true);
    ...
    assert.equal(f.state.opens, f.state.closes);
    await f.service.run(f.context, false, () => false, f.accounts[0].id);
...
test('Visible account changes stop inactive watches and keep refresh revisions account-local', async () => {
  ...
  try {
    ...
    f.state.eventChanged = true; f.service.setVisibleAccount(f.accounts[1].id); await flush(); await f.advance(1000);
...
test('A confirmed new arrival publishes a repeatable alert, unread badge and foreground banner once', async () => {
  ...
  try {
    ...
    assert.equal(await f.service.run(f.context, false, () => false, f.accounts[0].id), true);
...
test('Count guard or record failures retain a successful arrival check and its notification', async () => {
  for (const stage of ['badgeBeginError', 'badgeRecordError']) {
    ...
    assert.equal(await f.service.run(f.context, true, () => false, f.accounts[0].id), true, stage);
