...
async function fixture({ table = true, committedReads = false, nativeWindowLimit = false } = {}) {
  ...
  const db = {
    ...
    querySql: async (sql, args = []) => {
      ...
      if (nativeWindowLimit && values.some(row => row.reduce((bytes, value) => bytes +
      ...
      return { goToFirstRow: () => { index = 0; return values.length > 0; }, goToNextRow: () => ++index < values.length,
        getString: n => values[index][n], getLong: n => values[index][n], getBlob: n => new Uint8Array(values[index][n]), close() {} };
    },
...
test('Pending unread overlays work with a legacy Unicode body larger than the native RDB row window', async () => {
  const f = await fixture({ nativeWindowLimit: true });
  try {
    const saved = { version: 1, savedAt: Date.now(), bodySavedAt: Date.now(), bodyDecoderRevision: 1,
      mail: { ...mail('large'), preview: 'Saved preview', textBody: 'Unicode 中文 😀 '.repeat(180000) } };
    const payload = JSON.stringify(saved);
    assert.ok(Buffer.byteLength(payload) > 2 * 1024 * 1024);
    f.sqlite.prepare("INSERT INTO mail_cache (account_id, kind, cache_key, payload) VALUES ('a', 'email', 'large', ?)").run(payload);
    await f.record(10);
    assert.equal(await f.badges.total((_account, value) => ({ ...value, keywords: ['$seen'] }), () => ['large']), 9);
    assert.equal(f.sqlite.prepare("SELECT payload FROM mail_cache WHERE cache_key='large'").get().payload, payload);
  } finally { f.close(); }
});
