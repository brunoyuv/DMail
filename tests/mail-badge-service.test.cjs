...
test('Queued refreshes reread current opt-in totals and recover after a platform failure', async () => {
  const f = fixture(), gate = deferred(); f.state.gate = gate.promise;
  const first = f.service.refresh(f.store); await new Promise(setImmediate);
  const next = Array.from({ length: 30 }, () => f.service.refresh(f.store)); f.state.count = 4; gate.resolve();
...
test('A same-store refresh burst coalesces before execution while separate completed refreshes still write', async () => {
  const f = fixture();
  await Promise.all(Array.from({ length: 30 }, () => f.service.refresh(f.store)));
  assert.deepEqual(f.state.writes, [12]);
  assert.equal(f.state.reads, 2, 'One fresh total plus its post-write ownership check');
  await f.service.refresh(f.store);
  assert.deepEqual(f.state.writes, [12, 12], 'Resume must repair a badge changed by a separate process');
});
