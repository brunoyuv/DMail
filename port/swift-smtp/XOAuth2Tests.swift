...
private final class SMTPWire {
    ...
    func transferMessage() throws {
        try receive(220); _ = try outbound()
        try receive(250, ["AUTH XOAUTH2"]); _ = try outbound()
        try receive(235); _ = try outbound()
        try receive(250); _ = try outbound()
        try receive(250); #expect(try outbound() == "DATA\r\n")
        try receive(354); #expect(try outbound() != nil)
    }
...
private enum SyntheticCleanupError: Error { case failed }
struct SMTPDeliveryCleanupTests {
    @Test func acceptedDataRemainsAcceptedWhenCleanupFails() async throws {
        let wire = try SMTPWire(); defer { wire.close() }
        try wire.transferMessage()
        #expect(!wire.result.succeeded && !wire.result.failed)
        try wire.receive(250)
        let outcome = try #require(wire.result.value)
        var cleanups = 0
        try await finishSMTPDelivery(outcome) {
            cleanups += 1
            throw SyntheticCleanupError.failed
        }
        #expect(cleanups == 1)
        #expect(wire.result.succeeded)
        #expect(try wire.outbound() == "QUIT\r\n")
        #expect(try wire.outbound() == nil)
    }
    @Test func droppedDataAcknowledgementKeepsUnconfirmedOutcome() async throws {
        let wire = try SMTPWire()
        try wire.transferMessage()
        wire.close()
        let outcome = try #require(wire.result.value)
        var cleanups = 0
        do {
            ...
            Issue.record("An unconfirmed delivery must not become successful")
        } catch SMTPError.deliveryUnconfirmed {
            #expect(cleanups == 1)
        } catch {
            Issue.record("Cleanup replaced the original delivery outcome")
        }
    }
}
