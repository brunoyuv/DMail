...
struct MessageDataTests {
    @Test func attachmentMimeIsIdenticalForDeliveryAndPrivateSentCopy() throws {
        let body = try appendingAttachments([CompositionAttachment(name: "中文.pdf", contentType: "application/pdf", data: Data([0,255,42]))], to: compositionBody(text: "See attachment"))
        let email = Email(sender: EmailAddress("sender@example.test"), recipients: [EmailAddress("reader@example.test")],
            blindCopied: [EmailAddress("private@example.test")], subject: "Attachment", body: body)
        let wire = try messageData(email), saved = try messageData(email, includeBcc: true)
        #expect(String(decoding: saved, as: UTF8.self).replacingOccurrences(of: "Bcc: private@example.test\r\n", with: "") == String(decoding: wire, as: UTF8.self))
        #expect(wire.range(of: body.part.data) != nil)
        #expect(saved.range(of: body.part.data) != nil)
    }
