...
enum ImapFailure: String, Error { case forbidden, accountReadOnly, messageNotFound, changeUnconfirmed, invalidArgument, unsafeEndpoint, authenticationRequired, certificate, network, watchRestart, queryChanged, invalidResponse, unsupported, messageTooLarge }
...
struct ImapNullable<T: Encodable>: Encodable {
    let value: T?
...
struct ImapAccount: Encodable { let id: String, name: String; let isPersonal = true, isReadOnly = false }
struct ImapSession: Encodable {
    let apiUrl: String, state = "imap", accounts: [ImapAccount]
    let primaryAccountId = "default", maxObjectsInGet = 50
}
struct ImapMailbox: Encodable {
    ...
    let maySetSeen = false, maySetKeywords = false, mayAddItems = false, mayRemoveItems = false
}
struct ImapAddress: Encodable { let name: String, email: String }
struct ImapAttachment: Encodable { let id: String, name: String, contentType: String; let size: Int; let sizeIsEncoded: Bool }
struct ImapAttachmentData: Encodable { let name: String, contentType: String, base64: String }
struct ImapEmail: Encodable {
...
struct ImapInboxCheck: Encodable {
