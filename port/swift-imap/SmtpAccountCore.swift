...
private struct SmtpAttachment: Decodable, Sendable {
    let id: String, name: String, contentType: String
    let size: Int
    let base64: String?
}

private func messageAttachments(_ input: SmtpRequest) throws -> [SMTP.CompositionAttachment] {
    let entries = input.attachments ?? []
    guard entries.count <= SMTP.maximumCompositionAttachments else { throw SMTPError.invalidMessage }
    var result: [SMTP.CompositionAttachment] = [], ids = Set<UUID>(), total = 0
    for entry in entries {
        guard let id = UUID(uuidString: entry.id), ids.insert(id).inserted else { throw SMTPError.invalidMessage }
        try SMTP.validateAttachment(name: entry.name, contentType: entry.contentType, size: entry.size)
        total += entry.size
        guard total <= SMTP.maximumAttachmentBytes else { throw SMTPError.invalidMessage }
        if input.operation == "validate" {
            guard entry.base64 == nil else { throw SMTPError.invalidMessage }
        } else {
            guard let encoded = entry.base64, encoded.utf8.count == ((entry.size + 2) / 3) * 4,
                  let data = Data(base64Encoded: encoded), data.count == entry.size else { throw SMTPError.invalidMessage }
            result.append(SMTP.CompositionAttachment(name: entry.name, contentType: entry.contentType, data: data))
        }
    }
    return result
}
...
private func sendMessage(_ input: SmtpRequest) async throws -> SmtpReply {
    ...
    let editableBody = try SMTP.compositionBody(text: input.text, forwardHtml: input.forwardHtml ?? "")
    let body = try SMTP.appendingAttachments(messageAttachments(input), to: editableBody)
    ...
    // Do not parse/decode the attached files merely to cache the accepted text.
    let readable = try? EmailBody(body: editableBody)
...
public func smtpAccountRequest(_ raw: UnsafePointer<CChar>?) -> UnsafeMutablePointer<CChar>? {
    ...
    if let raw, let json = String(validatingCString: raw), json.utf8.count <= 16 * 1024 * 1024,
