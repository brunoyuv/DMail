...
final class XOAuth2Handler: IMAPCommandHandler, @unchecked Sendable {
    ...
    private func fail(_ context: ChannelHandlerContext) {
        ...
        promise.fail(IMAPError.commandFailed("OAuth authentication failed"))
        context.close(promise: nil)
    }
    func channelInactive(context: ChannelHandlerContext) { fail(context); context.fireChannelInactive() }
    func errorCaught(context: ChannelHandlerContext, error: Error) { fail(context) }
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        case .authenticationChallenge(let challenge):
            ...
            payload.clear()
            context.writeAndFlush(wrapOutboundOut(.part(.continuationResponse(reply)))).whenFailure { _ in self.fail(context) }
        case .tagged(let response):
            guard response.tag == tag, phase == .sent, case .ok = response.state else { fail(context); return }
