...
struct XOAuth2Tests {
    ...
    @Test func socketLossAndMalformedExchangesAreNotCredentialRejections() throws {
        for response in [nil, "OTHER NO [AUTHENTICATIONFAILED] untrusted tag\r\n", "A1 BAD Invalid exchange\r\n", "A1 NO [UNAVAILABLE] Try later\r\n"] as [String?] {
            let wire = try OAuthWire(initial: true); defer { wire.close() }
            _ = try wire.outbound()
            if let response { try wire.inbound(response) } else { wire.close() }
            guard case .failure(let error)? = wire.outcome.result else { Issue.record("Exchange unexpectedly succeeded"); continue }
            #expect(!(error is AuthenticationFailure))
            if response == nil { #expect(error as? IMAPError == .serverDisconnected) }
        }
    }
    @Test func explicitTaggedOAuthRejectionKeepsOnlyASafeTypedFailure() throws {
        let wire = try OAuthWire(initial: true); defer { wire.close() }
        _ = try wire.outbound(); try wire.inbound("A1 NO [AUTHENTICATIONFAILED] synthetic-private-response\r\n")
        guard case .failure(let error)? = wire.outcome.result else { Issue.record("Rejection unexpectedly succeeded"); return }
        #expect(error as? AuthenticationFailure == .rejected)
        #expect(String(describing: error) == "Authentication rejected")
    }
}
