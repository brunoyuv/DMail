...
const encodingFlags = new Map(encodingCases.map(item => [item.uid, new Set(['\\Seen'])]));
function encodingFixture(socket, session, method, args, tag) {
  ...
  if (method === 'LIST') {
    ...
    socket.write('* STATUS "INBOX" (MESSAGES 3 UNSEEN 0 UIDVALIDITY 77 UIDNEXT 204)\r\n'); ok();
    ...
    const selected = uidFetch ? encodingCases.filter(item => item.uid >= start && item.uid <= end) : encodingCases.slice(start - 1, end);
    if (end < start || end - start > 2 || selected.length === 0 || (!uidFetch && (start < 1 || end > 3))) {
    ...
    for (const item of selected) {
      const sequence = encodingCases.indexOf(item) + 1;
      ...
      if (args.includes('BODYSTRUCTURE')) {
        socket.write(` BODYSTRUCTURE ("TEXT" "${item.subtype}" ("CHARSET" "${item.charset}") NIL NIL "${item.transfer}" ${Buffer.byteLength(item.data)} 1)`);
      }
...
module.exports = function handle(socket, session, method, args, tag, mime) {
  if (session.mode === 'encoding') { return encodingFixture(socket, session, method, args, tag); }
  ...
  if (method === 'LIST') {
    ...
    if (!match || !session.selects || (args.includes('BODY') && !full && !headerOnly && sections.length === 0)) {
