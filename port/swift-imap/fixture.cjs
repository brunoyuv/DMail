...
const previewFixture = require('./preview-fixture.cjs');
    ...
    socket.on('data', data => {
      ...
      while ((end = buffer.indexOf('\r\n')) >= 0) {
        ...
        if (method === 'CAPABILITY') {
          const statusCapabilities = session.mode === 'status-basic-only' ? ' UIDPLUS CONDSTORE' : session.mode === 'preview-server' ? ' PREVIEW' : '';
          ...
          const credentials = /^(?:"([a-z0-9-]+)"|([a-z0-9-]+)) (?:"fixture-password"|fixture-password)$/.exec(args);
          ...
          else { authenticated = true; if (session.mode === 'preview-server') socket.write(`* CAPABILITY IMAP4rev1 PREVIEW\r\n${tag} OK Authenticated\r\n`); else ok(); }
        } else if (!authenticated) {
          stats.violations.push('command before authentication'); socket.destroy(); return;
        } else if (previewFixture.handle(socket, session, method, args, tag) || inboxWatchFixture.handle(socket, session, method, args, tag) || inboxCheckFixture.handle(socket, session, method, args, tag) || flagFixture.handle(socket, session, method, args, tag) || corpus?.handle(socket, session, method, args, tag) || accountFixture(socket, session, method, args, tag, mime)) {
          continue;
        } else if (method === 'LIST') {
