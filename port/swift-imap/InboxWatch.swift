...
// EXISTS can repeat the selected mailbox's unchanged count. Track EXPUNGE as
// well so an arrival that restores the previous count still creates a hint.
struct InboxWatchChanges: Sendable {
    private var messageCount: Int?
    init(messageCount: Int?) { self.messageCount = messageCount }
    mutating func exists(_ count: Int) -> Bool {
        let previous = messageCount
        messageCount = count
        return previous != count
    }
    mutating func expunged() {
        if let count = messageCount { messageCount = max(0, count - 1) }
    }
}
    ...
    private func run() async {
        ...
        do {
            ...
            do {
                ...
                else { try await value.login() }
            } catch AuthenticationFailure.rejected { throw InboxWatchError.authenticationRequired }
            if accessToken == nil { try await value.refreshCapabilities() }
            guard value.capabilities.contains(.idle) else {
                lock.withLock { supported = false }
                try await value.shutdown(); finish(); return
            }
            let selected = try await value.examine(mailbox: Mailbox.Name("INBOX"))
            var changes = InboxWatchChanges(messageCount: selected.messageCount)
            ...
            for await event in events {
                ...
                case .status(let status):
                    if let count = status.messageCount, changes.exists(count) { lock.withLock { changed = true } }
                case .bye:
