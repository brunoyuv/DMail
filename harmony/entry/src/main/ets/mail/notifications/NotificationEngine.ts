...
export async function deliverPending(repository: NotificationRepository, environment: NotificationEnvironment,
  ...
  if (!current.enabled || current.noticeVersion <= current.acknowledgedVersion) { return; }
  ...
  try {
    ...
    for (const value of accounts) {
      ...
      try { check = await environment.check(value); }
        ...
        if (!environment.cancelled()) { await repository.complete(value.accountId, value.revision, null, environment.now()); }
        continue;
      }
