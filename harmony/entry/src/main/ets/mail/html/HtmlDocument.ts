...
function isQuoteWrapper(tag: string, name: string): boolean {
  ...
  for (const attribute of mailHtmlAttributes(tag)) {
    if (attribute.name === 'class') {
      return /(?:^|\s)(?:gmail_quote|yahoo_quoted|protonmail_quote)(?:\s|$)/i.test(attribute.value || '');
    }
  }
  return false;
}
...
export function foldMailQuotes(html: string, showLabel: string = 'Show quoted text', hideLabel: string = 'Hide quoted text'): string {
  ...
  for (const token of mailHtmlTokens(html)) {
    if (token.kind !== 'tag') { continue; }
    const tag = html.slice(token.start, token.end), name = token.name, closing = token.closing;
    if (closing) {
      ...
      if (positions && positions.length > 0) {
        ...
        if (quoteIndex >= 0) { ranges[quoteIndex].end = token.end; }
      ...
      if (quoteIndex >= 0) { ranges.push(new QuoteRange(token.start, token.end, wrapper)); }
      const positions = openPositions.get(name) || [];
...
function mailImageStyle(style: string): Map<string, string> {
  const values = new Map<string, string>();
  const source = style.replace(/"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|\/\*[\s\S]*?(?:\*\/|$)/g,
    (token: string): string => token.startsWith('/*') ? '' : token);
  ...
  for (let index = 0; index <= source.length; index++) {
    ...
    if (quote) {
      ...
      const declaration = /^([a-z-]+)\s*:([\s\S]*)$/i.exec(source.slice(start, index).trim());
      if (declaration) {
        const name = declaration[1].toLowerCase(), value = declaration[2].trim().toLowerCase();
        if (!/!important\s*$/.test(values.get(name) || '') || /!important\s*$/.test(value)) { values.set(name, value); }
      } else if (source.slice(start, index).trim()) { values.set('ambiguous', ''); }
      start = index + 1;
    }
  }
  return values;
...
// A failed tracking pixel is not a missing newsletter picture. Remove only
// individual remote img elements with explicit tiny dimensions or an inline
// important hiding rule. Do not blacklist their URL: another visible image
// may use the same bytes. Ancestors, classes and folded quotes stay untouched.
function removeNonContentMailImages(html: string): string {
  // Stylesheets and independent min-size/transform rules can enlarge even
  // important inline dimensions. Retain ambiguous sizing without a CSS cascade.
  const authorStyles = /<style\b/i.test(html);
  return replaceMailHtmlTokens(html, (token, tag: string): string => {
    if (token.kind !== 'tag' || token.closing || token.name !== 'img') { return tag; }
    const values = new Map<string, string>();
    for (const attribute of mailHtmlAttributes(tag)) {
      if (attribute.value !== undefined && !values.has(attribute.name)) { values.set(attribute.name, pictureReferences(attribute.value)); }
    }
    if (!/(?:^|[\s,])(?:https?:)?\/\//i.test(`${values.get('src') || ''} ${values.get('srcset') || ''}`.trim())) { return tag; }
    const style = mailImageStyle(values.get('style') || '');
    if (/^none\s*!important\s*$/.test(style.get('display') || '') ||
      /^hidden\s*!important\s*$/.test(style.get('visibility') || '')) { return ''; }
    if (authorStyles || Array.from(style.keys()).some((name: string): boolean =>
      !['width', 'height', 'display', 'visibility'].includes(name))) { return tag; }
    const dimensions = ['width', 'height'].map((name: string): number => {
      const value = (style.get(name) ?? values.get(name) ?? '').trim().replace(/!important$/, '').trim();
      // Nonzero unitless CSS lengths are invalid, unlike HTML attributes.
      const valid = style.has(name) ? /^(?:(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)px|0)$/.test(value) :
        /^(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)(?:px)?$/.test(value);
      return valid ? Number(value.replace(/px$/, '')) : NaN;
    });
    return dimensions.every((value: number): boolean => value >= 0 && value <= 1) ? '' : tag;
  });
}
...
export function hasRemotePictures(html: string): boolean {
  ...
  const source = removeNonContentMailImages(html);
  ...
  for (const token of mailHtmlTokens(source)) {
    if (token.kind === 'comment' || token.closing || token.name === 'script') { continue; }
    if (token.kind === 'raw' && token.name === 'style' && hasRemoteCssPictures(source.slice(token.tagEnd, token.closeStart))) { return true; }
    const tag = source.slice(token.start, token.tagEnd), element = token.name;
    for (const attribute of mailHtmlAttributes(tag)) {
      const name = attribute.name;
      const value = pictureReferences(attribute.value || '');
...
function fitTableWidths(html: string): string {
  return replaceMailHtmlTokens(html, (token, tag: string): string => {
    if (token.kind !== 'tag' || token.closing || token.name !== 'table') { return tag; }
...
function browserMailLinks(html: string): string {
  return replaceMailHtmlTokens(html, (token, tag: string): string => {
    if (token.kind !== 'tag' || token.closing || !['a', 'area'].includes(token.name)) { return tag; }
    const parts: string[] = [];
    let offset = 0;
    for (const attribute of mailHtmlAttributes(tag)) {
      if (attribute.name === 'target' || attribute.name === 'download') {
        parts.push(tag.slice(offset, attribute.start)); offset = attribute.end;
      }
    }
    parts.push(tag.slice(offset));
    const cleaned = parts.join('');
    const end = cleaned.endsWith('/>') ? cleaned.length - 2 : cleaned.length - 1;
    return `${cleaned.slice(0, end).trimEnd()} target="_self">`;
  });
}
...
export function prepareMailHtml(html: string, showQuoteLabel: string = 'Show quoted text', hideQuoteLabel: string = 'Hide quoted text'): string {
  const body = replaceMailHtmlTokens(browserMailLinks(removeNonContentMailImages(html)), (token, tag: string): string =>
    token.kind === 'tag' && !token.closing && token.name === 'meta' ? '' : tag);
  return foldMailQuotes(fitTableWidths(body), showQuoteLabel, hideQuoteLabel);
}
